...
import {
  generateBeerpongTournament,
  getBeerpongParticipants,
  getBeerpongTournament,
  getPublicSettings,
  getReservations,
  getTableAvailability,
  getUmbrellaAvailability,
  resetBeerpongTournament,
  updateBeerpongMatchWinner,
  updatePaymentStatus,
...
export default function AdminPage({ settings, onSettingsChanged }) {
  ...
  const [tournamentState, setTournamentState] = useState({ tournament: null, matches: [] });
  const [tournamentTitle, setTournamentTitle] = useState('Torneo Beer Pong');
  ...
  async function refreshData() {
    ...
    setError('');
    try {
      const [reservationRows, umbrellas, tables, participants, tournament] = await Promise.all([
        getReservations(),
      ...
      if (tournament?.tournament?.title) setTournamentTitle(tournament.tournament.title);
    } catch (err) {
  ...
  async function handleGenerateTournament() {
    ...
    setError('');
    try {
      await generateBeerpongTournament({ title: tournamentTitle, participants: beerpongParticipants });
      await refreshData();
    } catch (err) {
      setError(err.message || 'Errore nella generazione del tabellone.');
    } finally {
      setTournamentSaving(false);
    }
  }

  async function handleSelectWinner(match, teamName) {
    if (!teamName) return;
    setTournamentSaving(true);
    setError('');
    try {
      await updateBeerpongMatchWinner(match.id, teamName);
      await refreshData();
    } catch (err) {
      setError(err.message || 'Errore aggiornamento vincitore.');
    } finally {
      setTournamentSaving(false);
    }
  }
  ...
  async function handleResetTournament() {
    ...
    setError('');
    try {
      await resetBeerpongTournament();
      await refreshData();
    } catch (err) {
      setError(err.message || 'Errore reset torneo.');
    } finally {
      setTournamentSaving(false);
    }
  }
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div>
                <div className="flex items-center gap-3"><Trophy className="h-5 w-5 text-slate-900" /><h3 className="text-xl font-semibold text-slate-900">Gestione tabellone Beer Pong</h3></div>
                <p className="mt-2 text-sm text-slate-600">Genera il tabellone a eliminazione diretta e clicca sui nomi delle squadre vincenti per farle avanzare.</p>
              </div>
              <a href="?page=torneo" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-700"><Tv2 className="h-4 w-4" />Apri vista display</a>
            </div>
            <div className="mt-5 grid gap-3 md:grid-cols-[1fr,auto,auto]">
              <Input value={tournamentTitle} onChange={(e) => setTournamentTitle(e.target.value)} placeholder="Titolo torneo" />
              <button onClick={handleGenerateTournament} disabled={tournamentSaving || beerpongParticipants.length < 2} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white disabled:opacity-60"><WandSparkles className="h-4 w-4" />Genera tabellone</button>
              <button onClick={handleResetTournament} disabled={tournamentSaving || !tournamentState.tournament} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-700 disabled:opacity-60"><RotateCcw className="h-4 w-4" />Reset</button>
            </div>
            <div className="mt-6">
              <BeerpongBracket tournament={tournamentState.tournament} matches={tournamentState.matches} editable onSelectWinner={handleSelectWinner} title={tournamentTitle} />
            </div>
          </div>
        </div>
